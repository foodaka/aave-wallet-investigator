...
// Component to handle search params that needs Suspense
function SearchParamsHandler({ onWalletAddressFound }: { onWalletAddressFound: (address: string) => void }) {
  ...
  useEffect(() => {
    ...
    if (urlWalletAddress && isAddress(urlWalletAddress)) {
      onWalletAddressFound(urlWalletAddress);
    }
  }, [searchParams, onWalletAddressFound]);

  return null; // This component doesn't render anything
}

function WalletInvestigatorContent() {
  const [walletAddress, setWalletAddress] = useState('');

  const handleWalletAddressFromUrl = useCallback((address: string) => {
  ...
  return (
    <>
      <Suspense fallback={null}>
        <SearchParamsHandler onWalletAddressFound={handleWalletAddressFromUrl} />
      </Suspense>
...
export default function WalletInvestigator() {
  return <WalletInvestigatorContent />;
}